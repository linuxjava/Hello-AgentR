...
- [Backend / Knowledge](./docs/backend/context/knowledge/CONTEXT.md) — 管理端治理的 KnowledgeBase（本阶段仅容器）与 YAML 配置的 ModelProvider / EmbeddingModel 只读目录；V0.2 引入，目录由模拟改为配置驱动
...
- **Frontend Admin → Knowledge**：管理端「知识库管理」消费 Knowledge API（列表 / 创建 / 改 Name·描述 / 删除 / EmbeddingModel 只读目录）；屏态见 Frontend Admin CONTEXT，实体定义仍只在 Backend / Knowledge。目录形态升级为对象列表后，管理端契约对齐属另一次变更。
