...
- [x] 3.2 工具栏：文件名模糊 + 状态 + 是否启用 +「查询」+「上传文档」；**无** strategy 筛选
- [x] 3.3 表格列严格按 Pencil：文件名（副行类型·大小）、状态（软色徽章；UPLOADED→「待分块」、CHUNKING→「处理中」、CHUNKED→「已就绪」、FAILED→「异常」）、分块数（未分块「—」）、启用开关、更新时间、改策略/删除
- [x] 3.4 分页默认 20；更新时间倒序
- [x] 3.5 空态 V-02、筛选空 V-03 文案与结构对照稿面；上传入口可用
- [x] 3.6 库不存在 V-04：错误态 + 返回列表；**禁止**做成可上传空列表
- [x] 3.7 文档列表页测试（列、空态、筛选空、状态/启用查询、A002001）
