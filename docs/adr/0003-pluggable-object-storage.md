...
**实现落点**：端口、S3 适配器与配置绑定在 `fw-base`（跨业务基建）。YAML 用 `type` 选择唯一活跃后端，S3 / OSS 参数分挂子块、只校验当前 type。Knowledge 只拥有 objectKey 约定（`{namespace}/{documentId}`），并把存储失败映射为管理端 `A002015`。fw-base 不得依赖 Document / Knowledge 错误码。
