...
- 独立文档列表（非知识库详情页）：文件名模糊、状态、是否启用、分页；列与状态展示以 Pencil 为准
...
| V-03 | State | `P-05 / V-03 Document List Filter Empty` | P-05 | 筛选无匹配 | 改条件；上传仍可用 |
...
| O-08a | Overlay | `O-08a Upload Structure Aware` | P-05 | 上传中切到「文档结构分块(建议MD文档使用)」 | 同 O-08 |
...
- **结构**
  ...
  - 工具栏：文件名模糊（占位见该帧）+ 状态 + 是否启用 +「查询」+ 主按钮「上传文档」
  - **无** strategy 筛选；状态 / 是否启用点「查询」才生效，缺省为全部
  ...
  - 状态：**毛玻璃胶囊徽章**（底 `$glass-fill` + 描边 `$edge-dim`，与行操作「删除」同形同底；字色按状态区分、无色点）；本阶段运行时多为「待分块」；帧内含四态示例（待分块 / 处理中 / 已就绪 / 异常）
  - 分块数：未分块为「—」，不用 0
  - 启用：行内开关（示例含关闭行）
  - 行操作：「改策略」「删除」
  - 分页：默认 20 条/页（上限 100）
...
| 后端 | 界面文案 | 徽章 |
| ---- | -------- | ---- |
| UPLOADED | 待分块 | 毛玻璃底 `$glass-fill` / 橙字 `#DE9139` |
| CHUNKING | 处理中 | 毛玻璃底 `$glass-fill` / 蓝字 `#4379ED` |
| CHUNKED | 已就绪 | 毛玻璃底 `$glass-fill` / 青绿字 `#33A985` |
| FAILED | 异常 | 毛玻璃底 `$glass-fill` / 红字 `#E04D4D` |
...
- **结构**：搜索框与状态下拉、是否启用下拉保留当前未提交/已提交条件（示例见该帧）；表头保留；空提示「暂无匹配的文档」（文案以该帧为准）；上传入口仍可用。
...
- **结构（定稿）**
  ...
  - 分块策略：下拉，默认「重叠分块」；另一选项文案为「文档结构分块(建议MD文档使用)」
  - 分块大小 `512`、重叠长度 `64`（字段旁**不**标注单位）
  - 策略 / 数字字段：输入盒内侧左侧装饰图标（策略 `layers`、大小 `hash`、重叠 `git-compare`）；数字直接键入无前导 `0`、无原生步进器
  - 操作：取消 / 上传
- **规则**
  - 单文件；拖拽或点击；已选后可拖拽替换或点击重选
  - 同名允许新增，不二次确认
  - 不等式：`chunkSize > 0` 且 `0 ≤ overlap < chunkSize`
  - 下拉打开与点选**无**水波纹 / 波浪；输入盒焦点高亮对齐 antd outlined Input（见 §6）
- **手势**
...
- **与 O-08 的差**：策略展示值为「文档结构分块(建议MD文档使用)」（枚举仍为 `STRUCTURE_AWARE`）；字段为最小 / 默认 / 最大 / 重叠，预填 `256` / `512` / `1024` / `32`；无 JSON 文本框。大小类字段图标同为 `hash`，重叠仍为 `git-compare`。
...
- **结构**
  - 标题：改策略
  - 文件名：主名可编辑 + 后缀只读（示例 `handbook` + `.pdf`）
  - 分块策略 + 数字字段：**回填已存值**（帧示例重叠分块 `400` / `80`，**不是**上传预填 512/64）；控件约定与 O-08 相同（图标、无前导 `0`、无下拉波浪、焦点高亮）
  - 切种类时字段整份替换为目标种类预填（与上传切种类相同）；切到结构分块时下拉文案同 O-08a
  - 操作：取消 / 保存
  - **无** JSON 文本框
- **手势**
...
| 手势 | 结果 |
| ---- | ---- |
| 保存成功 | 关弹窗；Toast；`updatedAt` 刷新；列表顺序与文件名反映更新 |
| 主名为空 / 含路径符 | 弹窗内错误；不提交 |
| 校验/业务失败 | 弹窗内错误；不关 |
| 取消 | 不提交 |
...
7. **策略下拉文案**：`STRUCTURE_AWARE` 界面为「文档结构分块(建议MD文档使用)」，不写「基于文档结构的分块」。
8. **下拉无波浪**：`SelectMenu` 触发器与选项点击不展示按钮水波纹，并关闭 Ant Design Wave。
9. **数字字段**：清空后保持空；键入过程不得拼出前导 `0`（如 `0111`）。
10. **输入焦点**：玻璃输入盒 `:focus-within` 高亮对齐 Ant Design outlined Input（主色描边 + 浅色外圈）。
...
| 文件名主名为空 | 弹窗内顶部红条 | 请输入文件名 |
| 文件名含路径符 | 弹窗内顶部红条 | 文件名不能包含 / \\ : |
| 更换后缀（后端） | 弹窗内顶部红条 | 不能修改文件名后缀 |
...
| 2026-08-14 | 策略文案改为「文档结构分块(建议MD文档使用)」；补字段图标、数字无前导 0、下拉无波浪、输入焦点高亮 |
