...
6. **鉴权**：Sa-Token + Redis；管理端使用独立 `loginType=admin`（`StpAdminUtil`）。`AdminSaTokenConfig` 拦截 `/admin/**`（排除 `/admin/auth/login`）。写操作在 Service 内按角色能力矩阵校验（`requireAdmin`）。Document 路径挂在 `/admin/knowledge-bases/**` 下，复用同一拦截器。
