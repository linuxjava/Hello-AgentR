...
├── fw-base/                                 # 基础框架：统一响应、异常、MyBatis-Plus、分布式 ID、鉴权基础设施、对象存储
...
| `fw-base` | `fw-base`     | 跨业务共享基建：统一返回体、错误码与异常、全局异常处理、MyBatis-Plus 配置、Snowflake 分布式 ID、Redis / Sa-Token 依赖承载、可插拔 ObjectStorage |
...
| S3 兼容对象存储 | Document 源文件（本地可用 MinIO；部署级单一活跃后端） | `hello-agentr.object-storage` |
...
psql -h "$DB_HOST" -U "$DB_USERNAME" -d hello-agent -f backend/app/src/main/resources/db/t_knowledge_document.sql
```

`t_knowledge_document.sql` 含 `CREATE TABLE IF NOT EXISTS` 与增量 `ALTER`（如 `enabled`）。已有库请执行完整脚本，避免漏列。
...
业务按 **领域模块** 分包，新功能优先落在 `com.xgc.agent.rag.features.<module>/`，职责划分：
...
flowchart TB
  ...
  subgraph backend["backend"]
    APP[app<br/>业务 + 启动]
    FW[fw-base<br/>统一响应 / 异常 / MP / ID / 鉴权基建 / ObjectStorage]
  end
...
4. **持久化**：MyBatis-Plus + PostgreSQL；分页插件按 `DbType.POSTGRE_SQL`；插入/更新自动填充 `createTime` / `updateTime`。AdminUser 与 Document **物理删除**（实体无 `@TableLogic`）。
...
6. **鉴权**：Sa-Token + Redis；管理端使用独立 `loginType=admin`（`StpAdminUtil`）。`AdminInterceptorConfig` 拦截 `/admin/**`（排除 `/admin/auth/login`）。写操作在 Service 内按角色能力矩阵校验（`requireAdmin`）。Document 路径挂在 `/admin/knowledge-bases/**` 下，复用同一拦截器。
7. **对象存储**：端口与 S3 适配器在 `fw-base`（`ObjectStorage`）；YAML `hello-agentr.object-storage.type` 选唯一活跃后端，`s3` / `oss` 参数分块、只校验当前 type。本阶段 `type` 必须为 `s3`（`oss` 启动失败）。Knowledge 只生成 objectKey（`{namespace}/{documentId}`）并把失败映射为 `A002015`。首次 put/delete 时若桶不存在则自动创建（不挡启动）。默认桶名 `hello-agentr-knowledge`。决策见 [ADR-0003](../adr/0003-pluggable-object-storage.md)。
8. **媒体类型**：上传以 Apache Tika 探测为准（落在 `features.knowledge.detect`，不进 `fw-base`）。
...
│
├── fw-base/
│   ├── pom.xml                             # hutool、MyBatis-Plus、Redis、Redisson、Sa-Token、AWS SDK S3
│   └── src/main/
...
│       │   └── storage/                    # ObjectStorage 端口、S3 适配器、Properties、工厂、ObjectStorageException
│       └── resources/lua/snowflake_init.lua
│
└── app/
    ├── pom.xml                             # 依赖 fw-base、web、postgresql、pgvector、validation、crypto、Tika
    └── src/
        ├── main/
        │   ├── java/com/xgc/agent/rag/
        │   │   ├── HelloAgentApplication.java
        │   │   ├── config/WebConfig.java   # UTF-8 消息转换、CORS
        │   │   └── features/               # 业务域（Package by Feature）
        │   │       ├── admin/              # 管理端身份与账号
        ...
        │   │       │   ├── config/AdminInterceptorConfig.java
        │   │       │   ├── bootstrap/BootstrapAdminInitializer.java
        │   │       │   ├── error/AdminErrorCode.java
        │   │       │   └── util/           # 凭证规则、BCrypt
        │   │       └── knowledge/          # 知识库容器 + Document 元数据（不含切块执行）
        │   │           ├── controller/     # KnowledgeBase / Document / EmbeddingModel
        │   │           ├── service/ + impl/
        │   │           ├── dao/entity + mapper/  # KnowledgeBaseDO、KnowledgeDocumentDO
        │   │           ├── dto/
        │   │           ├── chunk/          # ChunkStrategyParams 校验（不执行切块）
        │   │           ├── detect/         # Tika MIME 白名单
        │   │           ├── properties/     # ModelCatalogProperties（YAML 目录）
        │   │           ├── error/KnowledgeErrorCode.java
        │   │           ├── storage/ObjectKeys.java  # objectKey 约定；存储端口在 fw-base
        │   │           └── util/           # KnowledgeFieldRules、OriginalFilenameRules
        │   └── resources/
        │       ├── application.yaml
        │       └── db/                     # t_admin_user.sql、t_knowledge_base.sql、t_knowledge_document.sql
        └── test/java/com/xgc/agent/rag/
            ├── BackendApplicationTests.java
            ├── features/
            │   ├── admin/                  # 凭证规则 + 持久化 + 认证/治理 IT
            │   └── knowledge/              # 字段规则、目录绑定、Document 单测、占用打桩、持久化/API IT
            └── mybatisplus/                # CodeGenerator、探路样例（非正式规格）
```

`fw-base` 另有 `src/test/.../storage/ObjectStoragePropertiesValidationTest.java`（`type` / 子块结构校验）。
...
| **storage**       | 部署级可插拔对象存储（首版 `s3`；`oss` 占位） | `ObjectStorage`、`S3ObjectStorage`、`ObjectStorageProperties`、`ObjectStorageConfiguration`、`ObjectStorageException` |

### 业务模块现状

| 模块                   | 状态       | 说明                                       |
| -------------------- | -------- | ---------------------------------------- |
| 启动与配置                | ✅ 已接入    | `HelloAgentApplication`、`application.yaml`（端口 / 上下文 / PG / Redis / 上传 / ObjectStorage / 模型目录 / Sa-Token）、`WebConfig`（UTF-8 / CORS） |
| `fw-base` 基建         | ✅ 已接入    | 统一响应、按端错误码与异常、MP、Snowflake、Sa-Token / Redis、ObjectStorage（S3；缺桶自动创建） |
| 管理端身份与账号 `features.admin`     | ✅ V0.1   | 登录 / 登出 / me / 改己密；AdminUser 列表 / 创建 / 重置密码 / 改角色 / 物理删除；Bootstrap 初始化；`loginType=admin` 拦截器。详见 [版本更新](版本迭代/版本更新.md)、[api.md](api.md) |
| 知识库 `features.knowledge`     | ✅ V0.2–V0.4 | KnowledgeBase CRUD、YAML EmbeddingModel 目录、Document 上传/列表/改策略/启用禁用/删除（不含切块执行与下载）。见 [api.md](api.md)、[Knowledge 词汇](context/knowledge/CONTEXT.md)、[V0.4 PRD](版本迭代/V0.4/prd.md) |
...
测试位于 `app` 模块（包路径镜像业务包）；ObjectStorage 结构校验在 `fw-base`。
...
│       ├── OriginalFilenameRulesTest.java
│       ├── ChunkStrategyParamsValidatorTest.java
│       ├── MediaTypeDetectorTest.java
│       ├── ModelCatalogPropertiesBindingTest.java
│       ├── StaticEmbeddingModelCatalogTest.java
│       ├── CountingDocumentPresenceTest.java
...
fw-base/src/test/java/com/xgc/agent/framework/base/storage/
└── ObjectStoragePropertiesValidationTest.java
```
...
```bash
cd backend
./mvnw -pl fw-base,app -am test
./mvnw -pl fw-base -am test -Dtest=ObjectStoragePropertiesValidationTest -Dsurefire.failIfNoSpecifiedTests=false
./mvnw -pl app -am test -Dtest=AdminCredentialRulesTest,AdminUserPersistenceIT,AdminAuthAndUserIT,KnowledgeFieldRulesTest,KnowledgeBaseServiceImplTest,DocumentServiceImplTest,KnowledgeBasePersistenceIT,KnowledgeBaseIT -Dsurefire.failIfNoSpecifiedTests=false
```
...
| Apache Tika（tika-core + tika-parsers-standard-package） | ✅      | 上传 MIME 探测（AllowedMediaType） |
| AWS SDK v2 `s3`                          | ✅      | `fw-base` ObjectStorage 首版适配器；版本跟 Spring Boot BOM |
| JUnit                                    | ✅ test | 单元 / 集成测试                     |
...
| 2026-08-13 | ObjectStorage 端口与 S3 适配器下沉到 `fw-base`；Knowledge 仅保留 objectKey 约定 |
| 2026-08-14 | 对齐 V0.4：Document DDL/包结构、Tika、嵌套 ObjectStorage YAML、缺桶自动创建、默认桶 `hello-agentr-knowledge` |
