...
7. **对象存储**：`fw-base` `ObjectStorage`（首版 S3）；业务注入端口。Document 的 objectKey 仍由 Knowledge 生成。
...
│       │   └── storage/                    # ObjectStorage 端口、S3 实现、配置绑定
...
| **storage**       | 部署级可插拔对象存储（首版 s3）     | `ObjectStorage`、`S3ObjectStorage`、`ObjectStorageProperties` |
...
| `fw-base` 基建         | ✅ 已接入    | 统一响应、按端错误码与异常、MP、Snowflake、Sa-Token / Redis、ObjectStorage |
...
| 知识库容器 `features.knowledge`     | ✅ V0.2   | KnowledgeBase CRUD、Namespace / EmbeddingModel 绑定、模拟目录；无 Document / 索引。见 [api.md](api.md)、[V0.2 PRD](版本迭代/V0.2/prd.md) |
